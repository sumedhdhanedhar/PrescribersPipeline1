...
def top_5_Prescribers(df_fact_sel):
    try:
        ...
        df_presc_final = df_fact_sel.select("presc_id","presc_fullname","presc_state","country_name","years_of_exp","trx_cnt","total_day_supply","total_drug_cost").filter((df_fact_sel.years_of_exp >= 20) & (df_fact_sel.years_of_exp <= 50)).withColumn("dense_rank",dense_rank().over(spec)).filter(col("dense_rank") <= 5).select("presc_id","presc_fullname","presc_state","country_name","years_of_exp","trx_cnt","total_day_supply","total_drug_cost")
    except Exception as exp:
